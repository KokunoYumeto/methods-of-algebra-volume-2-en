...
\begin{center}\begin{tabular}{|c|l|l|l|} \hline
	Coface & $\mathrm{d}^i = \mathrm{d}^{n, i}: [n-1] \hookrightarrow [n]$ & $0 \leq i \leq n$ & monotone injection omitting only $i\in[n]$ \\ \hline
	Codegeneracy & $\mathrm{s}^j = \mathrm{s}^{n, j}: [n+1] \twoheadrightarrow [n]$ & $0 \leq j \leq n$ & monotone surjection taking the value $j\in[n]$ twice \\ \hline
\end{tabular}\end{center}
