...
def main():
    ...
    repo=get(API).json();pages_commit=get(f"{API}/commits/gh-pages").json()["sha"]
    rows=[];errors=[]
    ...
    checks={"publication_boundary":publication.get("result")=="PUSHED_PENDING_ANONYMOUS_READBACK",
            ...
            "public_gh_pages_branch":bool(pages_commit),
    ...
    out={"schema":"o014-english-github-public-readback-v1","verified_at_utc":datetime.now(timezone.utc).isoformat(),"result":result,
      ...
      "note":"GitHub's Pages-configuration API requires authentication; anonymous proof uses the public gh-pages commit plus exact HTTPS Pages bytes."}
